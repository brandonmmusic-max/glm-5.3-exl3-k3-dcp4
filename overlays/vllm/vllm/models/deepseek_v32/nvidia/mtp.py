...
class DeepseekV32MultiTokenPredictorLayer(nn.Module):
    def __init__(self, vllm_config: VllmConfig, prefix: str) -> None:
        ...
        self.mtp_recycle_mode = vllm_config.speculative_config.mtp_recycle_mode
        logger.info("MTP hidden-state recycle mode: %s", self.mtp_recycle_mode)
        self.config = config
    ...
    def forward(
        ...
        if is_sequence_parallel:
            hidden_states, pre_norm_residual = self.shared_head.norm(
                hidden_states, residual
            )
            if self.mtp_recycle_mode == "post_norm":
                # Preserve the established path: one hidden-width gather.
                hidden_states = sp_all_gather(hidden_states)[: positions.shape[0]]
            else:
                # The negative-control cell needs both values.  Gather them in
                # one collective, accepting the doubled payload only there.
                gathered = sp_all_gather(
                    torch.cat((hidden_states, pre_norm_residual), dim=-1)
                )[: positions.shape[0]]
                hidden_states, pre_norm_residual = gathered.chunk(2, dim=-1)
        else:
            # The MoE output is left un-reduced; fuse its all-reduce into the
            # final norm, as the main model does at layer boundaries.
            hidden_states, pre_norm_residual = fused_allreduce_rms_norm(
                hidden_states, residual, self.shared_head.norm
            )
        recycled_hidden_states = (
            hidden_states
            if self.mtp_recycle_mode == "post_norm"
            else pre_norm_residual
        )
        return hidden_states, recycled_hidden_states


class DeepseekV32MultiTokenPredictor(nn.Module):
    supports_mtp_recycle_mode = True
