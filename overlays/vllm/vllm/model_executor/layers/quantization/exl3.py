...
_LEGACY_MCG_SENTINEL = 0x9E3779B9
...
# The dedicated projection-mixed R7 ABI consumes FP32 route weights.
...
class Exl3Config(QuantizationConfig):
    ...
    def _require_enforce_eager(self) -> None:
        if self.rank_sliced_metadata is not None or (
            os.environ.get("VLLM_EXL3_UNIFORM_FIXED_FUSED", "0") == "1"
            and bool(getattr(self, "r7_routed_experts", None))
        ):
            # The routed-expert fast path is eagerly planned before graph
            # capture. The guarded uniform-R7 mode lifts every routed EXL3
            # payload into that same rank-sliced ABI before forward; the
            # checkpoint's non-routed payload remains unquantized. Only the
            # large-M parity fallback remains eager.
            return
...
class Exl3LinearMethod(LinearMethodBase):
    ...
    def _validate_marker(tensor: torch.Tensor, expected: int, name: str) -> None:
        ...
        if value != expected:
            if (
                name == "mcg"
                and expected == _MCG_SENTINEL
                and value == _LEGACY_MCG_SENTINEL
            ):
                logger.warning_once(
                    "Accepting legacy EXL3 MCG sentinel 0x%08x; "
                    "the payload remains the MCG codebook",
                    value,
                )
                return
            raise ValueError(
...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: RoutedExperts) -> None:
        ...
        if getattr(layer, "exl3_r7_graph", False):
            ...
            # Some image lineages carry the R7 graph-side Python patch without
            # carrying the matching exl3_moe_r7_fused extension symbol.  In
            # that mismatched state the B12X path is not an optional tuning
            # choice: it is the only executable R7 implementation, so neither
            # the diagnostic enable flag nor its layer budget may disable it.
            has_r7_graph_kernel = hasattr(
                _load_exl3_ext(), "exl3_moe_r7_fused"
            )
            force_b12x = not has_r7_graph_kernel
            # Diagnostic correctness oracle for the only four three-tier
            # layers in TR3v4.  The ordinary per-expert implementation is not
            # CUDA-graph safe, so this is deliberately opt-in and eager-only;
            # it lets the exact KLD harness distinguish a three-tier packing /
            # kernel defect from a checkpoint or attention regression.
            split = self._r7_projection_tiers(layer)
            if os.environ.get("VLLM_EXL3_UNIFORM_FIXED_FUSED", "0") == "1":
                # Uniform checkpoints do not need projection-mixed runtime-K
                # dispatch or ExLlama's expert-streaming wrapper.  Lift the R7
                # payload into the same direct one-grid B12X Trellis contract
                # used by the proven GLM-5.2 rank-sliced path.
                self._prepare_r7_uniform_fixed_weights(layer)
                return
            reference_layers = {
                value.strip()
                for value in os.environ.get(
                    "VLLM_EXL3_R7_THREE_TIER_REFERENCE_LAYERS", ""
                ).split(",")
                if value.strip()
            }
            layer_name = str(getattr(layer, "layer_name", ""))
            layer_selected = not reference_layers or any(
                f".layers.{value}." in layer_name for value in reference_layers
            )
            # Exact-arithmetic A/B selector for any R7 MoE layer.  This is an
            # eager-only quality probe: it deliberately leaves the original
            # per-expert EXL3 tensors in place and bypasses B12X repacking, so
            # exact end-to-end KLD can isolate execution arithmetic from the
            # checkpoint bytes.  Examples: "6,7,8" or "all".
            exact_layer_tokens = {
                value.strip().lower()
                for value in os.environ.get(
                    "VLLM_EXL3_R7_EXACT_LAYERS", ""
                ).split(",")
                if value.strip()
            }
            exact_layer_selected = bool(exact_layer_tokens) and (
                bool(exact_layer_tokens & {"all", "*"})
                or any(
                    f".layers.{value}." in layer_name
                    for value in exact_layer_tokens
                )
            )
            if force_b12x and exact_layer_selected:
                layer.exl3_r7_graph = False
                logger.warning(
                    "R7 exact per-expert arithmetic A/B selected for %s; "
                    "this quality-probe path is eager-only",
                    layer.layer_name,
                )
                return
            if (
                force_b12x
                and os.environ.get("VLLM_EXL3_R7_THREE_TIER_REFERENCE", "0") == "1"
                ...
                layer.exl3_r7_graph = False
                logger.warning(
                    "R7 three-tier eager reference path enabled for %s; "
                    "this diagnostic path is not CUDA-graph safe",
                    layer.layer_name,
                )
                return
            # r17 ships exl3_moe_fused but not exl3_moe_r7_fused. Its
            # graph-capable route is the pre-planned B12X mixed-Trellis path,
            # including mixed-Trellis3 for the four three-tier layers. Keep
            # the selection explicit until its end-to-end KLD gate is complete,
            # but do not misclassify r17 itself as graph-incapable merely
            # because the ordinary per-expert diagnostic branch is eager-only.
            if force_b12x and split is not None and len(split[0]) == 3:
                if os.environ.get("VLLM_EXL3_R7_GROUPED_EXPERIMENTAL", "0") == "1":
                    self._prepare_r7_grouped_weights(layer)
                    layer.exl3_r7_grouped = True
                    return
                if os.environ.get(
                    "VLLM_EXL3_R7_MIXED_TRELLIS3", "0"
                ) != "1":
                    layer.exl3_r7_graph = False
                    logger.warning(
                        "R7 three-tier exact per-expert path selected for %s; "
                        "set VLLM_EXL3_R7_MIXED_TRELLIS3=1 for the r17 "
                        "full-CUDA-graph qualification route",
                        layer.layer_name,
                    )
                    return
                logger.info(
                    "R7 three-tier r17 qualification routes %s through "
                    "graph-planned B12X mixed-Trellis3",
                    layer.layer_name,
                )
            if (force_b12x or _r7_fused_enabled()) and self._prepare_r7_b12x_weights(
                layer, ignore_layer_budget=force_b12x
            ):
                layer.exl3_r7_fused = True
                return
            if force_b12x:
                raise RuntimeError(
                    "R7 execution cannot continue: exl3_moe_r7_fused is not "
                    "exported by exllamav3_ext and this layer could not be "
                    "prepared for the B12X mixed-Trellis fallback"
                )
    ...
    def _trellis_tile_config(hidden_size: int, intermediate_size: int):
        ...
        # GLM-5.3 TP4 decode (H=6144, I_tp=512): the K128/N128 FC1 tile
        # distributes its 64 routed mn-tiles across the 188-CTA split-K grid
        # materially better than K64/N256.  Keep FC2 at K64/N256; the wider
        # output tile remains fastest for the 6144-column down projection.
        # Exact K3/K5 graph-replay selection evidence is retained under
        # glm53-exl3-k3-r10/qualification/2026-09-01/route-schedule-ab.
        if hidden_size == 6144 and intermediate_size == 512:
    ...
    def _prepare_r7_grouped_weights(self, layer: RoutedExperts) -> None:
        """Build uniform-K expert groups for r17's native EXL3 MoE kernel.

        ``exl3_moe_fused`` accepts one K value per projection per launch, while
        R7 stores those values per expert.  Experts outside the active group
        map to the kernel's sentinel bucket, so each routed contribution is
        evaluated exactly once and every group accumulates into one FP32
        output tensor.
        """

        ext = _load_exl3_ext()
        if not hasattr(ext, "exl3_moe_fused"):
            raise RuntimeError(
                "R7 grouped execution requires exl3_moe_fused in exllamav3_ext"
            )
        pointer_specs = (
            ("w13", "trellis", "w1"),
            ("w13", "suh", "w1"),
        ...
        def projection_bits(group: str, shard_id: str) -> tuple[int, ...]:
            tensors = getattr(layer, f"{group}_trellis").exl3_tensors
            values = tuple(
                int(tensors[(expert_id, shard_id)].shape[2] // 16)
        ...
        gate_bits = projection_bits("w13", "w1")
        up_bits = projection_bits("w13", "w3")
        down_bits = projection_bits("w2", "w2")
        triples = tuple(zip(gate_bits, up_bits, down_bits))
        groups = []
        for bits in sorted(set(triples)):
            expert_ids = tuple(
                expert_id
                for expert_id, expert_bits in enumerate(triples)
                if expert_bits == bits
            )
            sentinel = len(expert_ids)
            expert_map = torch.full(
                (num_experts,), sentinel, dtype=torch.int64, device=device
            )
            expert_map[
                torch.tensor(expert_ids, dtype=torch.int64, device=device)
            ] = torch.arange(sentinel, dtype=torch.int64, device=device)

            pointer_tables = []
            for group, attr, shard_id in pointer_specs:
                tensors = getattr(layer, f"{group}_{attr}").exl3_tensors
                rows = [tensors[(expert_id, shard_id)] for expert_id in expert_ids]
                if any(not row.is_contiguous() for row in rows):
                    raise RuntimeError(
                        f"R7 {group}_{attr}/{shard_id} contains a "
                        "non-contiguous grouped tensor"
                    )
            ...
            groups.append(
                SimpleNamespace(
                    ...
                    pointer_tables=tuple(pointer_tables),
                )
            )

        if sum(len(group.expert_ids) for group in groups) != num_experts:
            raise AssertionError("R7 grouped execution did not partition every expert")
        layer.exl3_r7_grouped_groups = tuple(groups)
        logger.info(
            "R7 grouped EXL3 MoE %s: %s",
            layer.layer_name,
            ", ".join(
                f"K{group.bits}={len(group.expert_ids)}"
                for group in groups
            ),
        )

    def _prepare_r7_uniform_fixed_weights(self, layer: RoutedExperts) -> None:
        """Lift a uniform R7 layer into GLM-5.2's one-grid Trellis ABI.

        R7 stores one tensor per expert because its general format permits a
        different K for every projection and expert.  A uniform checkpoint
        has no need for that runtime dispatch.  Stack the already TP-sliced
        tensors into the rank-sliced BTX slabs consumed by B12X's fixed-K
        fused MoE, then reuse the established GLM-5.2 preparation and runtime
        path unchanged.
        """
        ...
        up_bits = projection_bits("w13", "w3")
        down_bits = projection_bits("w2", "w2")
        present = set(gate_bits) | set(up_bits) | set(down_bits)
        declared = set(
            getattr(self.quant_config, "r7_routed_experts", {}).get(
                "k_values", ()
            )
        )
        invalid = sorted(present - declared)
        if invalid:
            raise ValueError(
                f"R7 uniform fixed layer uses K values {invalid} not declared "
                f"by r7_routed_experts.k_values={sorted(declared)}"
            )
        if len(present) != 1:
            raise RuntimeError(
                "VLLM_EXL3_UNIFORM_FIXED_FUSED requires one K value across "
                f"gate/up/down in {layer.layer_name}; got {sorted(present)}"
            )
        bits = next(iter(present))

        def stack_parameter(param_name: str, *, shared: bool = False) -> None:
            param = getattr(layer, param_name)
            shard_ids = tuple(param.exl3_shard_ids)
            rows = 1 if shared else num_experts
            tensors = param.exl3_tensors
            template = tensors[(0, shard_ids[0])]
            backing_shape = (
                (len(shard_ids), rows, *template.shape)
                if len(shard_ids) > 1
                else (rows, *template.shape)
            )
            backing = torch.empty(
                backing_shape,
            ...
            for shard_index, shard_id in enumerate(shard_ids):
                for expert_id in range(rows):
                    source = tensors[(expert_id, shard_id)]
                    destination = (
                        backing[shard_index, expert_id]
                        if len(shard_ids) > 1
                        else backing[expert_id]
                    ...
                    if tuple(source.shape) != tuple(destination.shape):
                        raise ValueError(
                            f"uniform R7 slab shape changed in {param_name}"
                            f"[{expert_id},{shard_id}]: {tuple(source.shape)} "
            ...
            param.exl3_backing = backing

        stack_parameter("w13_trellis")
        stack_parameter("w2_trellis")
        stack_parameter("w13_suh", shared=True)
        stack_parameter("w13_svh")
        stack_parameter("w2_suh")
        stack_parameter("w2_svh", shared=True)

        layer.exl3_layer_bitrates = (bits,) * num_experts
        layer.exl3_mixed_bitrate = False
        layer.exl3_max_num_batched_tokens = int(
            layer.exl3_r7_max_num_batched_tokens
        )
        self._prepare_rank_sliced_weights(layer)

        # PreparedExperts owns the compact slabs and rotation tables now.
        # Drop the loader-side aliases so each layer has exactly one payload
        # owner, matching the established rank-sliced checkpoint path.
        for prefix in ("w13", "w2"):
            for suffix in ("suh", "svh", "trellis", "mcg", "mul1"):
                param = getattr(layer, f"{prefix}_{suffix}")
                param.exl3_tensors.clear()
        ...
        layer.exl3_rank_sliced = True
        layer.exl3_r7_graph = False
        logger.info(
            "Uniform fixed-K B12X one-grid Trellis %s: K%d experts=%d",
            layer.layer_name,
            bits,
            num_experts,
        )
    ...
    def _prepare_r7_b12x_weights(
        ...
        *,
        ...
        ``ignore_layer_budget`` is reserved for image/extension mismatches
        where the dedicated R7 graph kernel is unavailable and this path is
        therefore required for correctness rather than selected as a tuning
        policy.
        """
    ...
    def _r7_graph_runtime(
        ...
        # A single scheduler group is a diagnostic/reference mode: experts are
        # retired in stable expert order, so their FP32 output atomics cannot
        # race across groups.  Production keeps the extension's full
        # concurrency unless the operator explicitly narrows it.
        concurrency = min(
            max_concurrency,
            _positive_env_int("VLLM_EXL3_R7_CONCURRENCY", max_concurrency),
        )
        hidden_size = int(layer.exl3_hidden_size)
        intermediate_size = int(layer.exl3_intermediate_size_per_partition)
        num_experts = int(layer.local_num_experts)
        ...
        runtime = {
            ...
            "weight_sorted": torch.empty(
                # Preserve FP32 router probabilities through the fused expert
                # scatter.  Both the projection-mixed R7 extension and the
                # established fixed-K exl3_moe_fused ABI require an FP32
                # sorted-route workspace in this extension build.
                route_capacity,
    ...
    def _apply_r7_grouped(
        ...
        """Run mixed-R7 experts as uniform-K native EXL3 MoE groups."""
        ...
        if m > int(layer.exl3_r7_max_num_batched_tokens):
            raise ValueError(
                "R7 grouped batch exceeds its planned capacity: "
                f"m={m}, capacity={layer.exl3_r7_max_num_batched_tokens}"
            )
        output = torch.empty(
            (m, int(layer.exl3_hidden_size)), dtype=x.dtype, device=x.device
        ...
        ext = runtime["ext"]
        groups = layer.exl3_r7_grouped_groups
        for start in range(0, m, capacity):
            stop = min(start + capacity, m)
            rows = stop - start
            if x.dtype == torch.float16:
            ...
            out32.zero_()
            for group in groups:
                group_size = len(group.expert_ids)
                gate_k, up_k, down_k = group.bits
                ext.exl3_moe_fused(
                    xh,
                    out32,
                    topk_ids[start:stop],
                    topk_weights[start:stop],
                    group.expert_map,
                    runtime["expert_count"][: group_size + 1],
                    runtime["expert_offsets"][: group_size + 1],
                    runtime["token_sorted"],
                    runtime["weight_sorted"],
                    runtime["tg"],
                    runtime["tu"],
    ...
    def _rank_sliced_runtime(
        ...
        topk = int(topk_ids.shape[1])
        device_index = x.device.index
        layer_bitrates = tuple(int(k) for k in layer.exl3_layer_bitrates)
        if not layer_bitrates or len(set(layer_bitrates)) != 1:
            raise ValueError(
                "rank-sliced runtime requires one uniform trellis bitrate, got "
                f"{layer_bitrates!r}"
            )
        trellis_bits = layer_bitrates[0]
        key = (
            ...
            int(layer.exl3_intermediate_size_per_partition),
            int(layer.local_num_experts),
            # A GLM in-tree MTP layer can retain the ordinary
            # ``model.layers.<N>`` name and share the target quant config, so
            # role/name inference alone cannot isolate its planner.  The B12X
            # weight plan and scratch geometry are bitrate-specific: the K5
            # draft must never reuse the target K3 plan.  Structural K keeps
            # one shared runtime for every same-K target layer while giving a
            # different-K draft exactly one additional runtime per rank.
            trellis_bits,
            topk,
            max_batched_tokens,
            min_trellis_m,
        ...
        logger.info_once(
            "EXL3 rank-sliced K%d runtime planned: Trellis m=%d..%d block_m=%d, "
    ...
    def apply(
        ...
        if getattr(layer, "exl3_r7_fused", False):
            ...
            return output.reshape(*original_shape, output.shape[-1])
        if getattr(layer, "exl3_r7_grouped", False):
            output = self._apply_r7_grouped(layer, x_2d, weights, ids)
            return output.reshape(*original_shape, output.shape[-1])
        if getattr(layer, "exl3_r7_graph", False):
        ...
        route_weight_fp32 = (
            os.environ.get("VLLM_EXL3_ROUTE_WEIGHT_FP32", "0") == "1"
        )
        if not route_weight_fp32:
        ...
        for expert_id in range(layer.local_num_experts):
            ...
            contribution = (
                expert_output.to(torch.float32) * route_weight
                if route_weight_fp32
                else (expert_output * route_weight).to(torch.float32)
            )
            output.index_add_(
                0,
                token_ids,
