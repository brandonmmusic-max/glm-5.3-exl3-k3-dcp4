...
# 368-byte KV_FP8_ROPE=1 record, or the GLM-5.3 NoPE 304-byte record, and a
# B12X build with the matching writer/reader mode. Mutually exclusive with
# VLLM_NVFP4_MLA_SCALES_FILE.
_NVFP4_DYNAMIC_SCALE_REQUESTED = NVFP4_MLA_CACHE_FORMAT.dynamic_scale

# Absorbed MLA query width, q_head_dim = kv_lora_rank + qk_rope_head_dim:
#   GLM_NSA  (GLM-5.2)        512 + 64 = 576
#   GLM_NOPE (GLM-5.3-Flash)  512 +  0 = 512   (mla_use_nope)
_NSA_Q_HEAD_DIM = 576
_NOPE_Q_HEAD_DIM = 512


def _current_model_uses_nope_mla() -> bool | None:
    """Resolve semantic NoPE from model config when a config context exists.

    Cache-shape resolution can receive an adapter-facing physical head width,
    so that width is not authoritative for GLM-5.3. Return None only when the
    config context is genuinely unavailable, allowing legacy callers/tests to
    fall back to the supplied head size.
    """
    from vllm.config import get_current_vllm_config
    ...
    if text_config is None or not hasattr(text_config, "qk_rope_head_dim"):
        return None
    return int(text_config.qk_rope_head_dim) == 0

# Keep the dedicated GLM_NOPE NVFP4 path behind an explicit image-level gate.
# The local b12x build carries the rope-less gather, shared-memory, prefill, and
...
class B12xMLASparseBackend(AttentionBackend):
    ...
    def get_supported_head_sizes(cls) -> list[int]:
        # q_head_dim = kv_lora_rank + qk_rope_head_dim, so GLM_NSA is 576 and
        # NoPE (GLM-5.3-Flash, qk_rope_head_dim=0) is 512. 512 is admitted here
        # only so the NVFP4 NoPE record can be selected: supports_combination
        # rejects every other cache dtype at that width, because b12x has no
        # 512-wide fp8/bf16 MLA record and DSV4 also lands on q_head_dim 512,
        # so an unguarded 512 would read DSV4 geometry off a NoPE cache.
        return [_NOPE_Q_HEAD_DIM, _NSA_Q_HEAD_DIM]
    ...
    def supports_combination(
        ...
        # FAIL-CLOSED: q_head_dim 512 is ambiguous in b12x -- it is both the
        # NoPE contract (kv_lora_rank 512 + rope 0) and DSV4's (448 + 64). Only
        # the NVFP4 record width tells them apart, so refuse 512 for any other
        # cache dtype rather than let a NoPE model silently take DSV4 geometry.
        if head_size == _NOPE_Q_HEAD_DIM:
            if kv_cache_dtype != "nvfp4_ds_mla":
                return (
                    "B12X_MLA_SPARSE supports head_size=512 "
                    "(qk_rope_head_dim=0) only with "
                    f"kv_cache_dtype='nvfp4_ds_mla'; got {kv_cache_dtype!r}"
                )
            if not _GLM_NOPE_NVFP4_OPT_IN:
                # Do not let an ordinary image select a locally patched device
                # contract merely because the host-side width is compatible.
                return (
                    "B12X_MLA_SPARSE NoPE (head_size=512) NVFP4 requires the "
                    "dedicated rope-less b12x device build. Set "
    ...
    def get_kv_cache_shape(
        ...
        if cache_dtype_str == "nvfp4_ds_mla":
            ...
            if is_nope:
                # GLM_NOPE: 256 B E2M1 latent + 32 B E4M3 group-16 scales and
                # no RoPE tail at all -- a strict 288 B prefix of the GLM_NSA
                # record. The dynamic per-token FP32 latent scale widens it to
                # 304 B; there is no fp8-rope variant because there is no RoPE
                # sub-record to quantize.
                return (
                    num_blocks,
                    block_size,
                    304 if _NVFP4_DYNAMIC_SCALE_REQUESTED else 288,
                )
...
class B12xMLASparseMetadataBuilder(AttentionMetadataBuilder[B12xMLASparseMetadata]):
    ...
    supports_draft_decode_metadata_update: bool = True
    ...
    def __init__(
        ...
        # IndexPool models score compressed K rows, then expand the winners
        # back to request-relative token ids. Their index-cache page ids cannot
        # be reused as MLA-cache page ids, so DCP1 must remap those logical ids
        # through the MLA block table just like the DCP path does.
        self.indexer_outputs_logical = (
            int(getattr(self.model_config.hf_config, "index_kpool", 1) or 1) > 1
        )
        self.needs_index_mapping = (
            self.dcp_world_size > 1 or self.indexer_outputs_logical
        )
        ...
        if self.needs_index_mapping:
            ...
            if self.dcp_world_size > 1 and ckv_gather_requested:
    ...
    def update_draft_decode_metadata(
        ...
        """Refresh the DCP-local causal lengths inside a fused MTP graph.

        ``update_draft_inputs`` advances ``metadata.seq_lens`` between draft
        steps.  The B12X DCP path stages rank-local copies during metadata
        construction, so those persistent graph inputs must be refreshed
        before the next replayed sparse-MLA step.  DCP1 aliases the common
        sequence-length tensor directly and therefore needs no copy.
        """
        if self.dcp_world_size == 1 or metadata.num_decode_tokens == 0:
            return
        if (
            metadata.num_prefills != 0
            or metadata.max_query_len != 1
            or metadata.num_decode_tokens != metadata.num_reqs
        ):
            raise RuntimeError(
                "B12X fused draft metadata update requires uniform one-token "
                "decode rows"
            )

        if self.cp_kv_cache_interleave_size != 1:
            raise RuntimeError(
                "B12X fused draft metadata update supports DCP interleave 1 only"
            )
        global_seq_lens = metadata.seq_lens[: metadata.num_reqs]
        # For unit interleave, rank r owns positions r, r + world, ... .  Write
        # the closed-form count directly into persistent graph storage.  The
        # general helper materializes a device rank scalar and is therefore not
        # safe to call while CUDA stream capture is active.
        torch.add(
            global_seq_lens,
            self.dcp_world_size - 1 - self.dcp_rank,
            out=metadata.cache_seq_lens_per_req,
        )
        torch.div(
            metadata.cache_seq_lens_per_req,
            self.dcp_world_size,
            rounding_mode="floor",
            out=metadata.cache_seq_lens_per_req,
        )
        metadata.cache_seq_lens_per_token.copy_(metadata.cache_seq_lens_per_req)
...
class B12xMLASparseImpl(MLAAttentionImpl[B12xMLASparseMetadata]):
    ...
    def __init__(
        ...
        **mla_args,
        ...
        if self._kv_fp8_rope or self._kv_nope_nvfp4:
            try:
        ...
        self._nvfp4_dynamic_scale = bool(
            (self._kv_fp8_rope or self._kv_nope_nvfp4)
            and _NVFP4_DYNAMIC_SCALE_REQUESTED
        )
        if _NVFP4_DYNAMIC_SCALE_REQUESTED and not (
            self._kv_fp8_rope or self._kv_nope_nvfp4
        ):
            raise RuntimeError(
                "VLLM_NVFP4_MLA_DYNAMIC_SCALE=1 requires a compact GLM "
                "nvfp4_ds_mla record (got kv_cache_dtype="
        ...
        # GLM_NSA is 512 + 64 = 576; NoPE (GLM-5.3-Flash) is 512 + 0 = 512.
        ...
        physical_topk_capacity = int(self.topk_indices_buffer.shape[-1])
        self.topk_tokens = int(
            getattr(indexer, "topk_tokens", None)
            or getattr(hf_config, "index_topk", physical_topk_capacity)
        )
        if not 0 < self.topk_tokens <= physical_topk_capacity:
            raise RuntimeError(
                "B12X_MLA_SPARSE semantic top-k must fit the indexer buffer: "
                f"logical={self.topk_tokens}, capacity={physical_topk_capacity}"
            )
        parallel_config = vllm_config.parallel_config
        ...
        configured_index_kpool = int(
            getattr(indexer, "index_kpool", None)
            or getattr(hf_config, "index_kpool", 1)
            or 1
        )
        self.indexer_outputs_logical = configured_index_kpool > 1
        if indexer is not None:
            # R10's B12xDeepseekV32Indexer always binds
            # output_physical_slots=False and no longer stores that legacy
            # constructor field on the module.
            indexer_outputs_physical = bool(
                getattr(indexer, "output_physical_slots", False)
            )
            if self.indexer_outputs_logical and indexer_outputs_physical:
                raise RuntimeError(
                    "B12X_MLA_SPARSE IndexPool requires logical sparse-indexer "
                    "output so MLA-cache slots can be remapped independently"
                )
        else:
            # R10 supplies the shared top-k buffer without passing the indexer
            # module into this attention constructor. The paired B12X indexer
            # emits cache-physical slots for DCP1 and logical indices for DCP.
            indexer_outputs_physical = self.dcp_world_size == 1
        expects_physical_slots = self.dcp_world_size == 1 and not (
            self.indexer_outputs_logical
        )
        if indexer_outputs_physical != expects_physical_slots:
        ...
        if self.kv_cache_dtype == "nvfp4_ds_mla":
            if self.q_head_dim == _NOPE_Q_HEAD_DIM:
                # GLM-5.3 NoPE has no RoPE sub-record: its packed E2M1 latent
                # and E4M3 scales occupy 288 bytes (304 with the optional
                # per-token scale), matching get_kv_cache_shape above.
        ...
        self._decode_plan = _make_plan(
            ...
            self._kernel_num_heads,
            self._decode_max_rows,
            # Direct DCP A2A consumes token-major packed heads. Preserve the
            # established head-major layout for DCP1, where the outer MLA
            # projection can consume it without a distributed combine.
            head_major_output=self.dcp_world_size == 1,
        )
    ...
    def _materialize_mla_query(
        self,
        ql_nope: torch.Tensor,
        q_pe: torch.Tensor,
        output: torch.Tensor,
    ) -> None:
        """Join absorbed MLA query parts, specializing the rope-less ABI."""
        if self.qk_rope_head_dim == 0:
            if q_pe.shape[-1] != 0:
                raise ValueError(
                    "GLM NoPE query must carry a zero-width RoPE tensor; "
                    f"got shape {tuple(q_pe.shape)}"
                )
            if ql_nope.shape != output.shape:
                raise ValueError(
                    "GLM NoPE absorbed query must exactly match the B12X "
                    f"workspace: {tuple(ql_nope.shape)} != {tuple(output.shape)}"
                )
            output.copy_(ql_nope)
            return
        ops.concat_mla_q(ql_nope, q_pe, output)
    ...
    def forward_mqa(
        ...
        if isinstance(q, tuple):
            ...
            self._materialize_mla_query(ql_nope, q_pe, q_all)
        else:
        ...
        if use_ckv_gather:
            ...
            _mask_page_table_after_nsa_len(selected_indices, nsa_cache_seqlens)
        elif not self.indexer_outputs_logical:
            ...
            # IndexPool returns request-relative token ids even without DCP.
            # Its compressed index-cache block table is not the MLA block
            # table, so remap through the latter into graph-stable scratch.
            assert attn_metadata.req_id_per_token is not None
            assert attn_metadata.page_table_1 is not None
            selected_indices = attn_metadata.page_table_1[
                :num_actual_toks, : topk_indices.shape[1]
        ...
        if use_decode_kernel:
            ...
            if self.need_to_return_lse_for_decode:
                ...
                if self._pad_heads:
                    if self.dcp_world_size > 1:
                        # The token-major decode plan may retain an aligned
                        # head-capacity pitch. Slicing the padded tail keeps
                        # heads packed (stride(1) == head_dim), which is the
                        # direct DCP reducer's explicit ABI; a head-major copy
                        # would violate it during engine warmup.
                        out = out[:, : self._input_num_heads, :]
                    else:
