...
class B12xIndexerMetadataBuilder(DeepseekV32IndexerMetadataBuilder):
    ...
    def update_draft_decode_metadata(
        ...
        """Refresh sequence lengths and the paged schedule for fused MTP.

        Draft input preparation advances the common global sequence lengths in
        place.  B12X decode consumes a separate persistent rank-local view and
        a schedule derived from that view, so both must be updated inside the
        captured multi-step graph.
        """
        decode = metadata.decode
        ...
        if metadata.num_decode_tokens != metadata.num_decodes:
            raise RuntimeError(
                "B12X fused draft indexer update requires one token per request"
            )

        num_decodes = metadata.num_decodes
        target_seq_lens = decode.seq_lens.reshape(-1)
        if target_seq_lens.numel() != num_decodes:
            raise RuntimeError(
                "B12X fused draft indexer sequence-length view is not one row "
                "per request"
            )

        global_seq_lens = metadata.seq_lens[:num_decodes]
        if self.dcp_world_size > 1:
            if self.cp_kv_cache_interleave_size != 1:
                raise RuntimeError(
                    "B12X fused draft indexer update supports DCP interleave 1 only"
                )
            torch.add(
                global_seq_lens,
            ...
            target_seq_lens.copy_(global_seq_lens)
...
class B12xSparseIndexer(nn.Module):
    def __init__(
        ...
        # DCP merges rank-local candidates into global logical token indices.
        # Without DCP, B12X sparse MLA consumes cache-physical slots directly.
        self.output_physical_slots = self.dcp_world_size == 1
