...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    @torch.inference_mode()
    def profile_prefill_extremes(self) -> None:
        """Profile real token- and request-maximal text prefills."""
        from vllm.v1.worker.gpu.cudagraph_utils import (
        ...
        try:
            ...
            # Recurrent-state outputs scale with request count, not just token
            # count. Exercise the admitted request maximum with real attention
            # so the post-KV scheduler warmup cannot reveal a larger GDN peak.
            hidden_states, sample_hidden_states = self._dummy_run(
                self.max_num_tokens,
                skip_eplb=True,
                is_profile=True,
    ...
    @torch.inference_mode()
    def capture_model(
        self, *, channel_id: str = "vllm:target:production"
    ) -> int:
