...
MTPIndexReuseMode = Literal["checkpoint", "on", "off"]
MTPRecycleMode = Literal["post_norm", "pre_norm_residual"]
...
class SpeculativeConfig:
    ...
    mtp_index_reuse: MTPIndexReuseMode = "checkpoint"
    """Control MTP sparse-index reuse across recurrent draft steps.

    ``checkpoint`` follows ``index_share_for_mtp_iteration`` from the draft
    checkpoint, while ``on`` and ``off`` provide explicit diagnostic controls.
    """

    mtp_recycle_mode: MTPRecycleMode = "post_norm"
    """Select the hidden state fed back to the next recurrent MTP step.

    ``post_norm`` is the production path. ``pre_norm_residual`` is a diagnostic
    negative control using the globally reduced residual before final RMSNorm.
    """
    ...
    def compute_hash(self) -> str:
        ...
        if self.method == "mtp":
            ...
            resolved_reuse = (
                checkpoint_reuse
                if self.mtp_index_reuse == "checkpoint"
                else self.mtp_index_reuse == "on"
            )
            factors.extend((resolved_reuse, self.mtp_recycle_mode))
    ...
    def _verify_args(self) -> Self:
        ...
        if self.method != "mtp" and (
            self.mtp_index_reuse != "checkpoint"
            or self.mtp_recycle_mode != "post_norm"
        ):
            raise ValueError(
                "mtp_index_reuse and mtp_recycle_mode are only valid with "
                "method='mtp'."
            )
        if self.method == "mtp" and self.num_speculative_tokens == 1 and (
            self.mtp_index_reuse != "checkpoint"
            or self.mtp_recycle_mode != "post_norm"
        ):
            raise ValueError(
                "MTP1 does not execute recurrent index reuse or hidden-state "
                "recycling; leave both controls at their defaults."
            )
