...
def _w4a16_small_m_blocks_per_sm() -> int:
    """Request one or two resident small-M CTAs, subject to resource limits.

    One preserves the qualified schedule. Two is experimental; the actual
    compiled register/shared-memory admission remains authoritative.
    """

    value = int(os.environ.get("B12X_W4A16_SMALL_M_BLOCKS_PER_SM", "1"))
    if value not in (1, 2):
        raise ValueError("B12X_W4A16_SMALL_M_BLOCKS_PER_SM must be 1 or 2")
    return value


def _w4a16_full_rotation_split_phases_enabled() -> bool:
    """Use four CUDA-graph-compatible launches instead of grid barriers."""

    return os.environ.get("B12X_W4A16_FULL_ROTATION_SPLIT_PHASES", "0") == "1"
...
def _determine_blocks_per_sm(
    *,
    ...
    force_small_m_residency = uses_m_block_8 and _w4a16_small_m_blocks_per_sm() > 1
    if (
        not force_small_m_residency
...
class W4A16FusedMoeKernel:
    def __init__(
        ...
        *,
        ...
        if self.compile_time_phase not in {-1, 0, 1, 2, 3}:
            raise ValueError("compile_time_phase must be -1 or one of 0, 1, 2, 3")
        if self.compile_time_phase >= 0 and not self.full_rotation:
            raise ValueError("split phase compilation requires full_rotation")
    ...
    def __call__(
        ...
        if cutlass.const_expr(
            self.full_rotation and self.compile_time_phase in {-1, 0}
        ):
        ...
        self.kernel(
            ...
            block=[self.cta_threads, 1, 1],
            min_blocks_per_mp=self.blocks_per_sm,
            # Split phases are ordered by stream/CUDA-graph edges and contain
            # no software all-grid barrier, so cooperative admission is only
            # required by the monolithic path.
    ...
    def _moe_body(
        ...
        if cutlass.const_expr(self.full_rotation):
            ...
            if cutlass.const_expr(self.compile_time_phase < 0):
                self._grid_barrier(locks_i32_flat, tid, grid_x)
        if cutlass.const_expr(
            self.tc_decode_fused_sum and self.compile_time_phase in {-1, 1}
        ):
        ...
        if cutlass.const_expr(self.activation_is_gated):
            if cutlass.const_expr(self.compile_time_phase in {-1, 1}):
            ...
            if cutlass.const_expr(self.compile_time_phase < 0):
                self._grid_barrier(locks_i32_flat, tid, grid_x)
            if cutlass.const_expr(
                self.full_rotation and self.compile_time_phase in {-1, 2}
            ):
                if cutlass.const_expr(self.coupled_hadamard):
            ...
            elif cutlass.const_expr(self.compile_time_phase in {-1, 2}):
        ...
        elif cutlass.const_expr(self.compile_time_phase in {-1, 1}):
            self.fc1._run_persistent_gemm(
                a_bf16_flat,
                a_alt_bf16_flat,
                w13_i32_flat,
        ...
        if cutlass.const_expr(self.compile_time_phase < 0):
            self._grid_barrier(locks_i32_flat, tid, grid_x)
        if cutlass.const_expr(self.compile_time_phase in {-1, 3}):
...
def compile_w4a16_fused_moe(
    *,
    ...
    result = W4A16FusedMoeCompileResult(
        ...
        compile_time_phase=kernel.compile_time_phase,
    )
...
def _w4a16_fused_moe_launch_flat(
    ...
    persistent_grid_x = _w4a16_fused_persistent_grid_x(
            fused=fused,
            m=m,
            topk=topk,
            intermediate_size=intermediate_size,
    ...
    if full_rotation and _w4a16_full_rotation_split_phases_enabled():
        compile_kwargs = dict(
            size_m=size_m,
            hidden_size=hidden_size,
            intermediate_size=intermediate_size,
        ...
        phase_launches = tuple(
            compile_w4a16_fused_moe(**compile_kwargs, compile_time_phase=phase)
            for phase in range(4)
        )
        warps_per_cta = max(int(fused.cta_threads) // 32, 1)
        input_block = 512 if coupled_hadamard else 128
        input_units = m * topk * (hidden_size // input_block)
        activation_units = m * topk * (intermediate_size // 128)
        phase_grids = (
            max((input_units + warps_per_cta - 1) // warps_per_cta, 1),
            persistent_grid_x,
            max((activation_units + warps_per_cta - 1) // warps_per_cta, 1),
            persistent_grid_x,
        )
        for phase_launch, phase_grid_x in zip(phase_launches, phase_grids):
            phase_launch.compiled(
                *launch_args,
                m,
                phase_grid_x,
                stream,
        ...
        fused.compiled(*launch_args, m, persistent_grid_x, stream)
