...
def atomic_add_global_acq_rel_i32(
    ...
    *,
    ...
    """GPU-scope acq_rel int32 atomic add. Returns the old value.

    Unlike a release fence followed by a relaxed reduction, this RMW imports
    the preceding value's release sequence before publishing the caller's
    stores.  It is used when multiple producer warps aggregate readiness into
    one counter and a later consumer must observe every producer's writes.
    """
    return Int32(
        llvm.inline_asm(
            T.i32(),
